...
x_vali=np.expand_dims(x_vali,3)
...
#fgsm_test.shape =(1052, 28, 28, 1)    

with np.load("./PGD_Test.npz") as f:
    pgd_test, pgd_test_labels = f['advs'], f['labels']
#pgd_test.shape (2264, 28, 28, 1)
    
with np.load("./FGSM_Vali.npz") as f:
    fgsm_vali, fgsm_vali_labels = f['advs'], f['labels']
#fgsm_vali shape 1193
#pgd_vali shape 2671

with np.load("./PGD_Vali.npz") as f:
    pgd_vali, pgd_vali_labels = f['advs'], f['labels']    


with np.load("./FOL_Fuzz_10mins.npz") as f:
    fol_fuzz_adv, fol_fuzz_labels , fol_fols = f['advs'], f['labels'] , f['fols']
...
#         s = strategies[i]
model_path = "./checkpoint/10mins/best_lenet_MIX_10mins_fol_{epoch:03d}.h5" 
...
x_train_mix = np.concatenate((x_train, fol_fuzz_adv),axis=0)
y_train_mix = np.concatenate((y_train, fol_fuzz_labels),axis=0)
        

        # load old model 
model = keras.models.load_model("./baseline/FMNIST_10_lenet5_model.010.h5")  
