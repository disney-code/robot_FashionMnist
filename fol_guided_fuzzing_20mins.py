...
seeds = random.sample(list(range(x_train.shape[0])), 20000)
...
for idx in seeds_filter:
    ...
    if delta_t > 1200:
        print(f"Time ran was (should be 1200seconds): {delta_t}")
        break
    img_list = []
    tmp_img = images[[idx]]
...
np.savez('./FOL_Fuzz_20mins.npz', advs=advs, labels=labels, fols=fols)
